...
func (suite *IntegrationTestSuite) initKeepersWithmAccPerms(blockedAddrs map[string]bool) (authkeeper.AccountKeeper, keeper.BaseKeeper) {
	app := suite.app
	maccPerms := sedaapp.GetMaccPerms()
...
func (suite *IntegrationTestSuite) TestSendCoinsNewAccount() {
	...
	suite.Require().NoError(sedaapp.FundAccount(app.BankKeeper, ctx, addr1, balances))
...
func (suite *IntegrationTestSuite) TestSendCoins() {
	...
	acc2 := app.AccountKeeper.NewAccountWithAddress(ctx, addr2)
	app.AccountKeeper.SetAccount(ctx, acc2)
	suite.Require().NoError(sedaapp.FundAccount(app.BankKeeper, ctx, addr2, balances))
...
func (suite *IntegrationTestSuite) TestHasBalance() {
	...
	suite.Require().NoError(sedaapp.FundAccount(app.BankKeeper, ctx, addr, balances))
...
func (suite *IntegrationTestSuite) TestMsgSendEvents() {
	...
	suite.Require().NoError(sedaapp.FundAccount(app.BankKeeper, ctx, addr, newCoins))
...
func (suite *IntegrationTestSuite) TestMsgMultiSendEvents() {
	...
	suite.Require().NoError(sedaapp.FundAccount(app.BankKeeper, ctx, addr, sdk.NewCoins(sdk.NewInt64Coin(fooDenom, 50))))
	suite.Require().Error(app.BankKeeper.InputOutputCoins(ctx, inputs, outputs))
	...
	suite.Require().NoError(sedaapp.FundAccount(app.BankKeeper, ctx, addr2, sdk.NewCoins(sdk.NewInt64Coin(barDenom, 100))))
	newCoins2 = sdk.NewCoins(sdk.NewInt64Coin(barDenom, 100))
...
func (suite *IntegrationTestSuite) TestSpendableCoins() {
	...
	app.AccountKeeper.SetAccount(ctx, vacc)
	app.AccountKeeper.SetAccount(ctx, acc)
	suite.Require().NoError(sedaapp.FundAccount(app.BankKeeper, ctx, addr1, origCoins))
	suite.Require().NoError(sedaapp.FundAccount(app.BankKeeper, ctx, addr2, origCoins))
...
func (suite *IntegrationTestSuite) TestVestingAccountSend() {
	...
	// receive some coins
	suite.Require().NoError(sedaapp.FundAccount(app.BankKeeper, ctx, addr1, sendCoins))
