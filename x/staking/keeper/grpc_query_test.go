...
func createValidators(t *testing.T, ctx sdk.Context, app *sedaapp.SedaApp, powers []int64) ([]sdk.AccAddress, []sdk.ValAddress, []types.Validator) {
	addrs := sedaapp.AddTestAddrsIncremental(app, ctx, 5, app.StakingKeeper.TokensFromConsensusPower(ctx, 300))
	valAddrs := sedaapp.ConvertAddrsToValAddrs(addrs)
