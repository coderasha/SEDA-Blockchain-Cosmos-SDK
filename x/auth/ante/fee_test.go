...
func (suite *AnteTestSuite) TestDeductFees() {
	...
	err = sedaapp.FundAccount(suite.app.BankKeeper, suite.ctx, addr1, coins)
	suite.Require().NoError(err)
	...
	suite.app.AccountKeeper.SetAccount(suite.ctx, acc)
	err = sedaapp.FundAccount(suite.app.BankKeeper, suite.ctx, addr1, sdk.NewCoins(sdk.NewCoin("atom", sdk.NewInt(200))))
	suite.Require().NoError(err)
