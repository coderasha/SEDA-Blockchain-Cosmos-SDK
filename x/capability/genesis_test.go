...
func (suite *CapabilityTestSuite) TestGenesis() {
	...
	db := dbm.NewMemDB()
	encCdc := sedaapp.MakeTestEncodingConfig()
	newApp := sedaapp.NewSedaApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, sedaapp.DefaultNodeHome, 5, encCdc, sedaapp.EmptyAppOptions{})
